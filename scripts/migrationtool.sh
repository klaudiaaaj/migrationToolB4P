    ...
    while IFS='|' read -r version current_folder current_name; do
        ...
        if [ -n "$target_line" ]; then
            ...
            if [ "$current_name" != "$target_name" ] ||
                ! folders_have_equal_cs_files "$current_folder" "$target_folder"; then
                error "VERSION_COLLISION_OR_MODIFIED_MIGRATION" \
                    "Wersja $version oznacza inną migrację w kodzie i w '$target_ref'. Source: '${version}_$current_name', target: '${version}_$target_name'. Zrób rebase albo uruchom sync."
            fi
            continue
        fi
